...
extension DiagnosticsExtension on LocalConnectAppState {
  ...
  Future<List<DiagnosticCheck>> runDiagnostics() async {
    ...
    // بلا فتح تلقائي فوق شاشة القفل — الرنين والاهتزاز يعملان، لكن لا يظهر
    // اسم/رقم المتصل إلا إذا فتح المستخدم التطبيق يدويًا بنفسه أولًا، وهذا
    // بالضبط أحد الأعراض المُبلَّغ عنها (المكالمة لا تظهر رغم سماع الجرس).
    final fullScreenIntentGranted = await callService.hasFullScreenIntentPermission();
    checks.add(DiagnosticCheck(
      label: 'صلاحية شاشة المكالمة الواردة الكاملة',
      ok: fullScreenIntentGranted,
      detail: fullScreenIntentGranted
