...
class GroupCallService extends ChangeNotifier {
  ...
  Future<void> _teardownAllConnections() async {
    ...
    // خطوتان ضروريتان معًا لتحرير الميكروفون فعليًا من منظور بقية النظام —
    // نفس الإصلاح المطبَّق في CallService._cleanupCallResources للمكالمات
    // الثنائية، وينطبق بنفس القوة هنا: إيقاف المسارات وحده لا يكفي؛ بدون
    // clearAndroidCommunicationDevice تحديدًا يبقى أندرويد يعتبر التطبيق
    // "في جلسة اتصال" على مستوى النظام، فتفشل أو تُسجَّل فارغة أي محاولة
    // تسجيل صوت لاحقة حتى من تطبيق آخر تمامًا.
    try {
