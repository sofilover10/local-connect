...
class MessagingSocketService {
  ...
  /// بلا حدود ويستنزف الذاكرة (رفض خدمة). لكن رسالة المرفق **كلها** سطر
  /// JSON واحد (لا تأطير ثنائي منفصل)، وترميز base64 يُضخِّم الحجم الأصلي
  /// بنحو الثلث — حد صغير هنا لا يمنع إساءة الاستخدام فقط، بل يقطع نقل
  /// مرفقات كبيرة (مثل ملف APK) بصمت في منتصف الطريق. 256 ميغابايت تكفي
  /// لملف أصلي حتى ~190 ميغابايت تقريبًا، وتبقى حدًا معقولًا لتطبيق مراسلة
  /// على شبكة محلية (ليس الأنسب لملفات أضخم من ذلك، كما هو موثَّق في
  /// README).
  static const int _maxBufferedBytes = 256 * 1024 * 1024;
